...
def rename_mp4_files(folder_path, new_names):
    # Get a list of all mp4 files in the current folder
    mp4_files = [file for file in os.listdir(folder_path) if file.endswith('.mp4')]

    # lesson006

    # Rename the first 4 files using the provided array of names
    for i in range(min(357, len(mp4_files))):
        
        old_file_name = mp4_files[i]
        fileIndex = old_file_name[6:9]
        new_file_name = f"{fileIndex}_{new_names[int(fileIndex)]}.mp4"
        
        # Construct the old and new file paths
        old_file_path = os.path.join(folder_path, old_file_name)
        new_file_path = os.path.join(folder_path+"/new", new_file_name)
        
        # Rename the file
        os.rename(old_file_path, new_file_path)

if __name__ == "__main__":
    # Specify the folder path where you want to search for files (current folder in this case)
    folder_path = os.getcwd()

    # Specify the array of new names
    file_names = [
        "Welcome",
"Intro",
...
"Singletons and Inject_Autowire",
"CLI Project Exercise ",
"SOLID Principles",
...
"Exercise Solution"
    ...
    # Call the function to rename the first 4 mp4 files
    rename_mp4_files(folder_path, file_names)
